...
function MainForm() {
  ...
  const handleImageClick = () => {
    if (!image && !temporaryImage) {
      // Open file dialog
      // Assuming you have a function to open file dialog, e.g., openFileDialog()
      openFileDialog();
    } else {
      // Toggle between temporary and original image
      setTemporaryImage(!temporaryImage);

      // Reset temporary image after 1 second
      if (temporaryImage) {
        setTimeout(() => {
  ...
  const getImageSource = () => {
    if (image) {
      ...
      return "/images/file_upload.png";
    }
  };

  useEffect(() => {
    setImageSource(getImageSource());
  }, [image, isActive, temporaryImage]);
  const ScrollToTop = () => {
    if (topRef.current) {
