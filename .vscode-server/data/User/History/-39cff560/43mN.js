...
const MedicineMap = () => {
  ...
  const toggleMarker = (subregion) => {
    ...
    const markers = positions.filter(
      (position) => position.subregion === subregion
    );
    markers.forEach((marker) => {
      if (isVisible[subregion]) {
        marker.marker.setMap(null); // 마커를 지도에서 제거
      } else {
        marker.marker.setMap(map); // 마커를 지도에 표시
      }
    });
  };
  ...
  return (
    <div className="NotDrag">
      ...
      >
        ...
        >
          <p>서구 마커 {isVisible["서구"] ? "숨기기" : "보이기"}</p>
        </div>
        <div
          style={{
            border: "solid #c8c8c8",
        ...
        >
          <p>달서구 마커 {isVisible["달서구"] ? "숨기기" : "보이기"}</p>
        </div>
      </div>
    </div>
  );
